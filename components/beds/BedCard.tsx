...
export function BedCard({ bed, onBook, onRelease, isBooking, isReleasing }: BedCardProps) {
  ...
  return (
    <div className="group card p-6 hover:shadow-xl transition-all duration-300 animate-scale-in">
      ...
      {bed.status === BedStatus.LOCKED && (
        <div className="mb-5 p-4 bg-gradient-to-r from-yellow-50 to-amber-50 border-l-4 border-yellow-400 rounded-lg">
          <div className="flex items-start">
            ...
            <p className="text-sm text-yellow-800 font-medium">Currently being booked by another user. Please choose another available bed.</p>
          </div>
        </div>
      )}
      ...
      {bed.status === BedStatus.OCCUPIED && onRelease && (
        <button
